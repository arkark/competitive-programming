...
int[][] query(int[] s, double d) {
    assert(s.length == N);
    assert(d>=1-1e-5 && d<=10+1e-5);
    s.map!(s => (s+'a').to!char).writeln;
    d.writeln;
    stdout.flush;
    return Q.rep!(() => readln.chomp.map!(c => (c-'a').to!int).array);
}
...
void main() {
    ...
    int[] xs = N.rep!(() => uniform(0, K, rnd));

    int T = 8;
    int[] ys = xs.map!(x => x.repeat(T).array).join.array;
    xs = xs[0..N/T];
    ys = ys[0..N];
    ...
    foreach(i; 0..Q) {
        ...
        writeln(max(0, min(N-1, maxJ*T + T)));
    }
}
