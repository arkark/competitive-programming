...
void main() {
    ...
    int[] as = [-1] ~ readln.split.to!(int[]).map!"a-1".array;
    ...
    foreach(bits; 0..1<<N) {
        foreach(i; 0..N) {
            ...
            if ((bits&sets[i]) != sets[i]) continue;
            long v = max(dp[bits].maxValue, dp[bits].nowValue + xs[i]);
            if (v < dp[bits|1<<i].maxValue) {
                dp[bits|1<<i].maxValue = v;
                dp[bits|1<<i].nowValue = dp[bits].nowValue + xs[i];
                foreach(j; 0..N) {
                    if (sets[i]&1<<j) dp[bits|1<<i].nowValue -= xs[j];
                }
            }
        }
    }
    dp.back.maxValue.writeln;
}
