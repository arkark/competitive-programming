...
void main() {
    ...
    auto solver = MaxFlow(2*N + 2);
    long s = 2*N;
    long t = 2*N+1;
    foreach(i; 0..N) {
        solver.addEdge(s, 0*N+i, 1);
        solver.addEdge(1*N+i, t, 1);
    }
    foreach(i, p; ps) foreach(j, q; qs) {
        if (p.x<q.x && p.y<q.y) {
            solver.addEdge(0*N+i, 1*N+j, 1);
        }
    }
    solver.solve(s, t).writeln;
}
