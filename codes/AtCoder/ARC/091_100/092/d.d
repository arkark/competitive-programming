...
void main() {
    ...
    long[] bs = readln.split.to!(long[]).sort!"a<b".array;
    long K = 29;

    long f(long x, long i) {
        return x&((1<<i)-1);
    }
    long[] cs = new long[K];
    foreach(i; 0..K) {
        long[] es = [0L] ~ bs.filter!(b => (b>>i&1)==0).array.sort!(
            (b1, b2) => f(b1, i) != f(b2, i) ? f(b1, i) < f(b2, i) : b1 < b2
        ).array;
        long[] fs = [0L] ~ bs.filter!(b => (b>>i&1)==1).array.sort!(
            (b1, b2) => f(b1, i) != f(b2, i) ? f(b1, i) < f(b2, i) : b1 < b2
        ).array;
        long E = es.length-1;
        long F = fs.length-1;
