...
void main() {
  ...
  foreach(i; 0..N-2) {
    vs[i].edges ~= Edge(vs[i], vs[i+1], 0L<<i);
    vs[i].edges ~= Edge(vs[i], vs[i+1], 1L<<i);
  }
  ...
  foreach(i; 0..N) {
    if (L>>i&1) {
      L = L>>(i+1)<<(i+1);
      if (i == N-1) {
        vs[i].edges ~= Edge(vs[i-1], vs[i], 0L<<(i-1));
        vs[i].edges ~= Edge(vs[i-1], vs[i], 1L<<(i-1));
      } else {
        vs[i].edges ~= Edge(vs[i], vs[N-1], L);
      }
    }
  }

  long M = vs.map!(v => v.edges.length).sum;
  writeln(N, " ", M);
  foreach(v; vs) {
    foreach(e; v.edges) {
      writeln(e.start.index+1, " ", e.end.index+1, " ", e.value);
    }
  }
}
