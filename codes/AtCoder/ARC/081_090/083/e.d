...
void main() {
    ...
    readln.split.to!(long[]).enumerate.each!(
        a => vs[a.value-1].children ~= vs[a.index+1]
    );
    ...
    void f(Vertex v) {
        foreach(c; v.children) {
            f(c);
        }
        long T = v.children.length;
        long[][] dp = new long[][](T+1, K);
        dp[0][] = 0;
        foreach(i; 0..T) {
            dp[i+1][] = INF;
            Vertex c = v.children[i];
            foreach(j; 0..K) {
                if (j+c.x<K) dp[i+1][j+c.x] = min(dp[i+1][j+c.x], dp[i][j]+c.value);
                if (j+c.value<K) dp[i+1][j+c.value] = min(dp[i+1][j+c.value], dp[i][j]+c.x);
            }
        }
        v.value = dp[T][v.x];
    }
    ...
    writeln(vs.front.value<INF ? "POSSIBLE" : "IMPOSSIBLE");
}
