...
void main() {
    ...
    long[] bs = readln.split.to!(long[]).map!"a-1".array;
    ...
    as = as.map!(a => aa[a]).array;
    bs = bs.map!(b => aa[b]).array;

    auto seg = SegTree!(long, (a, b) => a+b, 0L)(N);
    long[] ts = as.map!((a) {
        seg.update(a, seg.get(a) + 1);
        return seg.query(a+1, N);
    }).array;
    long T = ts.sum;
    if (T%2!=0) {
        writeln(-1);
    ...
    long[long] cc;
    long cnt = 0;
    as.enumerate.array.sort!"a.value>b.value".each!(
        (a) {
            if (S == T/2) return;
            long d = min(N-(a.index-ts[a.index])-1-cnt, S-T/2);
            S -= d;
            cc[a.value] = a.index-ts[a.index]+d;
            cnt++;
        }
    );
    long[] ds = new long[N];
    ...
    ds.map!(d => bb[d]).map!"a+1".map!(to!string).join(" ").writeln;
}
