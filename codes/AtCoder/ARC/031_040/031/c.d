...
void main() {
    ...
    auto seg1 = SegTree!(long, (a, b)=>a+b, 0L)(N);
    long[] cs = bs.map!((b) {
        long res = seg1.query(b, N);
        seg1.update(b, seg1.get(b) + 1);
        return res;
    }).array;

    auto seg2 = SegTree!(long, (a, b)=>a+b, 0L)(N);
    long[] ds = bs.retro.map!((b) {
        long res = seg2.query(b, N);
        seg2.update(b, seg2.get(b) + 1);
        return res;
    }).array.retro.array;

    zip(cs, ds).map!(a => min(a[0], a[1])).sum.writeln;
}
