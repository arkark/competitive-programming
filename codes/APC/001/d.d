...
void main() {
    ...
    auto trees = N.rep!(() => redBlackTree!("a<b", true, long)());

    foreach(i, p; N.iota.map!(i => uf.findSet(i)).array) {
        trees[p].insert(as[i]);
    }

    trees = trees.filter!"!a.empty".array;
    if (trees.length == 1) {
        0.writeln;
        return;
    }

    auto t1 = redBlackTree!("a<b", true, long)();
    auto t2 = redBlackTree!("a<b", true, long)();
    foreach(tree; trees) {
        t1.insert(tree.front);
        tree.removeFront;
        t2.insert(tree[]);
    }

    long ans = 0;
    ans += t1.front; t1.removeFront;
    ans += t1.front; t1.removeFront;
    while(!t1.empty) {
        if (t2.empty) {
            writeln("Impossible");
