...
void main() {
    ...
    long[] hs = N.rep!(() => readln.chomp.to!long);

    auto po = SegTree!(long, (a, b) => a>b ? a:b, 0)(hs);

    int countLeft(int index) {
        if (index==0) return 0;
        int l = 0;
        int r = index+1;
        while(l < r) {
            int c = (l+r)/2;
            long h = po.query(c, index+1);
            if (h > hs[index]) {
                if (l == c) break;
                l = c;
            } else {
    ...
    int countRight(int index) {
        if (index==N-1) return 0;
        int l = index;
        int r = N;
        while(l < r) {
            int c = (l+r)/2;
            long h = po.query(index, c+1);
            if (h > hs[index]) {
                r = c;
            } else {
                if (l == c) break;
    ...
    foreach(int i; 0..N) {
        writeln(countLeft(i) + countRight(i));
    }
