...
void main() {
    readln.split.to!(int[]).pipe!(
        ns => ns.length.to!int.rep!(
            () => readln.split.to!(long[]).sort!"a<b".map!(x => ModLong(x))
        ).enumerate.map!(
            xs => xs.value.enumerate.map!(
                a => ModLong(a.index)*a.value - ModLong(ns[xs.index]-a.index-1)*a.value
            ).sum
        ).reduce!"a*b"
    ).writeln;
}
