...
void main() {
    ...
    while(true) {
        ...
        /*rec(0, 0, kaizan, 0, 0, -10, -10, -10).array.length.writeln;*/
        char[][] ans = rec(0, 0, kaizan, 0, 0, -10, -10, -10);//.sort.uniq.array;
        ans.length.writeln;
        if (ans.length <= 5) {
...
char[][] rec(int depth, int d, char[] str, int flg, long th, int insertedIndex, int replacedIndex, int removedIndex) {
    char[][] result;
    if (depth == str.length) {
        if (d<D) {
            // insert back
            foreach(char c; set[min(depth, L-1)].byKey) {
                ...
                result ~= rec(depth, d+1, _str, flg, th, depth, replacedIndex, removedIndex);
            }
        }
        if (flg == ~(~0<<L)) result ~= str;
    } else {
        if (d<D) result ~= infect(depth, d, str, flg, th, insertedIndex, replacedIndex, removedIndex);
        flg = (flg<<1) & ~(1<<L);
        th = th*prime + str[depth];
        if (depth>=L) th -= str[depth-L]*prime^^L;
        if (depth>=L-1) flg = flg | rabinKarp(th);
        if (depth<L-1 || flg & 1<<(L-1)) {
            result ~= rec(depth+1, d, str, flg, th, insertedIndex, replacedIndex, removedIndex);
        }
    }
    return result;
}

char[][] infect(int depth, int d, char[] str, int flg, long th, int insertedIndex, int replacedIndex, int removedIndex) {
    char[][] result;
    // insert
    ...
    if (depth!=insertedIndex && depth!=insertedIndex+1 && depth!=replacedIndex && depth!=removedIndex) {
        foreach(char c; set[min(depth, L-1)].byKey) {
            ...
            char[] _str = str.dup;
            _str[depth] = c;
            result ~= rec(depth, d+1, _str, flg, th, insertedIndex, depth, removedIndex);
        }
    }
    // remove
    for (int i=0; i<1; i++) {
        ...
        result ~= rec(depth, d+1, _str, flg, th, insertedIndex, replacedIndex, depth);
    }
    return result;
}
