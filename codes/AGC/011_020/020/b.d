...
void main() {
    int K;
    scanln(K);
    long[] as = readln.split.to!(long[]).retro.array;
    ...
    long[2][] dp = new long[2][K];
    dp[0] = [2, 3];
    foreach(i; 1..K) {
        dp[i][0] = ceil(dp[i-1][0], as[i])*as[i];
        if (dp[i][0] > dp[i-1][1]) {
            writeln(-1);
            return;
        }
        dp[i][1] = (floor(dp[i-1][1], as[i]) + 1)*as[i] - 1;
    }
    writeln(dp.back[0], " ", dp.back[1]);
}
