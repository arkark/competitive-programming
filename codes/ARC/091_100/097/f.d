...
void main() {
    ...
    foreach(_; 0..N-1) {
        ...
        scanln(x, y);
        x--; y--;
        vs[x].edges ~= Edge(vs[x], vs[y]);
        vs[y].edges ~= Edge(vs[y], vs[x]);
    }
    long whiteCount = 0;
    Vertex s;
    ...
    auto po = () {
        ...
        s.used = true;
        s.dist = s.isWhite ^ (s.edges.length%2 != 0);
        s.prev = null;
        ...
        while(!list.empty) {
            ...
            foreach(e; v.edges) {
                ...
                if (u.used) continue;
                u.used = true;
                u.dist = v.dist + (u.isWhite ^ (u.edges.length%2 != 0));
                u.prev = v;
                list.insertFront(u);
            }
        }
        g = vs.filter!(v => v.dist<INF).maxElement!"a.dist";
    };
    ...
    foreach(v; vs) {
        ...
        ans += 2;
        ans += v.isWhite ^ (v.edges.length%2 !=  0);
    }
    ans -= 2;
    ans -= 2*g.dist;
    ans.writeln;
