...
void main() {
    ...
    long[] ds = new long[K];
    foreach(i; 0..K) {
        auto ss = str.filter!(s => cs[0..i].all!(c => s!=c)).array;
        if (ss.empty) break;
        cs[i] = ss.minElement.to!char;
        ds[i] = str.countUntil!(s => s==cs[i]);
    }
    ...
    foreach(i; 0..K) {
        ...
        auto tree = redBlackTree!("a<b", false, string)();
        foreach(j; 0..N) {
            if (str[j] != cs[i]) continue;
            foreach(k; 0..K) {
                if (j+k+1 > N) break;
                tree.insert(str[j..j+k+1]);
            }
        }
        tree[].array[K-1].writeln;
        break;
    }
}
