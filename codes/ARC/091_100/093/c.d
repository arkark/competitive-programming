...
void main() {
    long N;
    scanln(N);
    long[] as = [0L] ~ readln.split.to!(long[]) ~ [0L];
    long s = (N+1).iota.map!(i => abs(as[i+1] - as[i])).sum;
    long[] bs = N.iota.map!(i => abs(as[i+2] - as[i]) - abs(as[i+2] - as[i+1]) - abs(as[i+1] - as[i])).array;
    N.iota.map!(i => s+bs[i]).each!writeln;
}
