...
void main() {
    ...
    auto g = new Generator!long({
        foreach(i; 0..A) {
            yield((B-0-1)*A + i);
        }
        foreach(i; 0..A) foreach(j; 1..B) {
            yield((B-j-1)*A + i);
        }
    });
    long[] xs = g.take(N).array;
    int[long] aa;
    foreach(x; xs) {
        aa[x] = 1;
    }
    long[] keys = aa.keys.sort!"a<b".array;
    foreach(i; 1..N) {
        aa[keys[i]] += aa[keys[i-1]];
    }
    xs.map!(x => aa[x]).map!(to!string).join(" ").writeln;
}
