...
void main() {
    ...
    int[][] trie = new int[][](100010, 2);
    foreach(as; trie) as[] = -1;

    {
        int v = 1;
        N.times!({
            int u = 0;
            foreach(k; readln.chomp.map!"a=='0'?0:1") {
                u = trie[u][k]<0 ? trie[u][k] = v++ : trie[u][k];
            }
        });
    }

    long grundy(long height) {
        long res = 1;
        while(height%res == 0) {
            res *= 2;
        }
    ...
    ans.pipe!(a => a>0 ? "Alice" : "Bob").writeln;
