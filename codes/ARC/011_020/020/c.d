...
void main() {
    ...
    ModNum ten(long num) {
        if (num == 0) return ModNum(1);
        if (num%2!=0) {
            return memoize!ten(num-1)*10;
        } else {
            return memoize!ten(num/2)*memoize!ten(num/2);
        }
    }

    ModNum f(long base, long num)  {
        if (num == 0) return ModNum(0);

        long L = base.to!string.length;
        if (num%2!=0) {
            return memoize!f(base, num-1)*memoize!ten(L) + base;
        } else {
            return memoize!f(base, num/2)*memoize!ten(L*num/2)+memoize!f(base, num/2);
        }
    }
    ModNum ans = 0;
    foreach(i; 0..N) {
        ans += f(as[i], ls[i]);
        if (i<N-1) ans *= ten(as[i+1].to!string.length*ls[i+1]);
    }
    ans.writeln;
}

alias ModNum = ModNumber!(long);

struct ModNumber(T) if (__traits(isIntegral, T)) {
