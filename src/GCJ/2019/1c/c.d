module src.GCJ.2019.1c.c;import std.stdio;
import std.string;
import std.format;
import std.conv;
import std.typecons;
...
long mex(long[] xs) {
  foreach(i; 0..long.max) {
    if (i >= xs.length || i != xs[i]) return i;
  }
  assert(false);
}

long[][][][] dp = new long[][][][](15, 15, 16, 16);
long solve() {
  long R, C;
  scanln(R, C);
  bool[][] bss = R.rep!(()=>readln.chomp.map!"a=='#'".array);
  foreach(i1; 0..R) foreach(j1; 0..C) {
    foreach(i2; 0..R+1) foreach(j2; 0..C+1) {
      dp[i1][j1][i2][j2] = -1;
    }
  }

  long resultCount = -1;
  long grundy(long i1, long j1, long i2, long j2) {
    if (dp[i1][j1][i2][j2] >= 0) {
    ...
    foreach(i3; i1..i2) {
      ...
      if (!exists) {
        long x = grundy(i1, j1, i3, j2)^grundy(i3+1, j1, i2, j2);
        if (x == 0) cnt += j2 - j1;
        xs ~= x;
      }
    }
    foreach(j3; j1..j2) {
      ...
      if (!exists) {
        long x = grundy(i1, j1, i2, j3)^grundy(i1, j3+1, i2, j2);
        if (x == 0) cnt += i2 - i1;
        xs ~= x;
      }
    }
    resultCount = cnt;
    xs = xs.sort!"a<b".uniq.array;
    // [i1, j1, i2, j2, cnt, mex(xs)].writeln;
    return dp[i1][j1][i2][j2] = mex(xs);
  }
