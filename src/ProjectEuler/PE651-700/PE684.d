...
ModNum s(long n) {
  long t = n/9;
  return (ModNum(10)^^t - 1) + (ModNum(10)^^t * (n%9));
}

// T(t) := S(9*t)
// T(0) = 0, T(n) = 54*10^(n-1) - 9 + T(n-1)
ModNum T(long t) {
  return 6*(ModNum(10)^^t - 1) - ModNum(9)*t;
}

ModNum S(long k) {
  if (k < 1) return ModNum(0);
  if (k%9 == 0) return T(k/9);

  long t = (k-1)%9+1;
  long p = t*(t-1)/2;
  return p * ModNum(10)^^((k-1)/9) + t * s((k-1)/9*9+1) + S((k-1)/9*9);
}
