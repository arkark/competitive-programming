...
void main() {
    int A, B;
    scanln(A, B);
    auto as = readln.split.to!(int[]).redBlackTree!("a<b", false, int);
    auto bs = readln.split.to!(int[]).redBlackTree!("a<b", false, int);
    auto cs = redBlackTree!("a<b", false, int)();
    cs.insert(as[]);
    cs.insert(bs[]);

    int x = cs[].count.to!int;
    int y = cs[].map!((c) {
        if (as.empty || bs.empty) return 0;
        int a = as.front;
        int b = bs.front;
        if (a == c) as.removeFront;
        if (b == c) bs.removeFront;
        return c==a && c==b ? 1 : 0;
    }).sum;
    writefln("%.8f", y/cast(double)x);
}
