...
void main() {
    ...
    double ans = N.iota.map!((i) {
        Vector u = vs[i];
        Vector v = vs[(i+1)%$];
        Vector dir = normalize(v-u);
        return dot(u-p, Vector(-dir.y, dir.x)).abs;
    }).reduce!min;
...
struct Vector {
    double x, y;
    Vector opBinary(string op)(Vector that) {
        return Vector(mixin("this.x"~op~"that.x"), mixin("this.y"~op~"that.y"));
    }
    Vector opBinary(string op)(double that) {
        return Vector(mixin("this.x"~op~"that"), mixin("this.y"~op~"that"));
    }
}
