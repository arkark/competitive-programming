...
void main() {
    long H, W;
    readf("%d %d", &H, &W); readln;
    long[][] css = new long[][](H+1, W+1);
    foreach(i; 1..H+1) {
        css[i][1..$] = readln.split.to!(long[]);
    }
    foreach(i; 0..H+1) foreach(j; 0..W+1) {
        // css[i][j] *= (i+j)%2==0 ? 1 : -1; // <- NG
        css[i][j] = (i+j)%2==0 ? css[i][j] : -css[i][j];
    }
    foreach(i; 0..H+1) {
        foreach(j; 1..W+1) {
            css[i][j] += css[i][j-1];
    ...
    foreach(i1; 0..H) foreach(j1; 0..W) {
        foreach(i2; i1+1..H+1) foreach(j2; j1+1..W+1) {
            long s = 0;
            s += css[i2][j2];
            s -= css[i1][j2];
            s -= css[i2][j1];
