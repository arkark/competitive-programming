...
void main() {
  ...
  auto tree = redBlackTree!("a.i <  b.i", true, P)();
  ...
  long r1 = B;
  long l1 = r1 - C.floor(2);

  long r2 = -B;
  long l2 = r2 - (C - 1).floor(2);
  ...
  if (B <= 0) {
    ...
    add(-B, min(0, -B + (C - 1).floor(2)));
    add(-(-B), -(-B) + (C - 1 - 1).floor(2));
  }
  ...
  long preT = t;
  while (!tree.empty) {
    while (!tree.empty && tree.front.i == i) {
      t += tree.front.x;
      tree.removeFront;
    }
    if (preT > 0) {
