...
void main() {
  ...
  long[] as = N.floor(2).iota.map!(i => vs[2*i]).array;
  long[] bs = N.floor(2).iota.map!(i => vs[2*i+1]).array;
  ...
  auto f(long[] xs) {
    ...
    aa[-1] = 0;
    return aa.byPair.array.sort!"a[1]>b[1]".array;
  }
  ...
  long ans = INF;
  if (cs[0][0] != ds[0][0]) ans.ch!min(N - cs[0][1] - ds[0][1]);
  if (cs[0][0] != ds[1][0]) ans.ch!min(N - cs[0][1] - ds[1][1]);
  if (cs[1][0] != ds[0][0]) ans.ch!min(N - cs[1][1] - ds[0][1]);
  ans.writeln;
}
