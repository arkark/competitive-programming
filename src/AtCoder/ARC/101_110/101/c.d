...
void main() {
  ...
  long[] ys = xs.retro.filter!"a<0".map!"-a".array;
  long[] zs = xs.filter!"a>0".array;
  ...
  foreach(i, y; ys) {
    if (K-i-2 >= 0 && K-i-2 < zs.length) ans.ch!min(y*2 + zs[K-i-2]);
  }
  foreach(i, z; zs) {
    if (K-i-2 >= 0 && K-i-2 < ys.length) ans.ch!min(z*2 + ys[K-i-2]);
  }
  if (ys.length>=K) ans.ch!min(ys[K-1]);
  if (zs.length>=K) ans.ch!min(zs[K-1]);
  ans.writeln;
}
