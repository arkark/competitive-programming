...
void main() {
    int N;
    scanln(N);
    int[] ps = readln.split.to!(int[]);

    Vertex[] vs = (N+1).rep!(() => new Vertex);
    foreach(i, p; ps) {
        vs[i+1].parent = vs[p];
        vs[p].children ~= vs[i+1];
    }

    int[] depthCnt = new int[N+1];
    void calcDepthCnt(Vertex v, int d = 0) {
        depthCnt[d]++;
        v.children.each!(v => calcDepthCnt(v, d+1));
    }
    calcDepthCnt(vs.front);
