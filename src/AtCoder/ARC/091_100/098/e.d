...
void main() {
  ...
  long[][] bss = as.enumerate.array.sort!"a.value<b.value".chunkBy!"a.value == b.value".map!(map!"a.index").map!array.array.to!(long[][]);
  bss = [[]] ~ bss;

  long ans = INF;

  auto seg = SegTree!(long, "a+b", 0)(N);
  long[] ones = 1L.repeat(N).array;
  long[] ls = (-1L).repeat(N).array;
  long[] rs = N.repeat(N).array;
  foreach(i, bs; bss) {
    foreach(b; bs) {
      foreach(j; 0..b) rs[j].ch!min(b);
      foreach(j; b..N) ls[j].ch!max(b);
    }
    seg.update(ones);
    long q = 0;
    long minV = +INF;
    ...
    foreach(xs; bss[i+1..$]) {
      foreach(x; xs) {
        if (q == Q) break;
        long k = seg.query(ls[x]+1, rs[x]-1+1);
        if (k >= K) {
          seg.update(x, 0);
          q++;
          minV.ch!min(as[x]);
