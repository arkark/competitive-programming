...
void main() {
  with(Geometry2d!(double, 1e-5)) {
    ...
    Vec s = ps.reduce!"a+b" / N;
    Vec g = qs.reduce!"a+b" / N;
    ...
    foreach(i; 0..N) {
      if (qs[i].eq(Vec(0, 0))) continue;
      double theta = qs[i].arg - ps[0].arg;

      bool ok = true;
      foreach(j; 0..N) {
        ok &= qs.any!(
          q => q.eq(ps[j].rotate(theta))
        );
      }
      if (ok) {
        writeln("Yes");
