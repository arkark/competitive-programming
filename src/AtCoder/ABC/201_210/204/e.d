...
void main() {
  ...
  foreach(i; 0..M) {
    ...
    a--;
    b--;
    vs[a].edges ~= Edge(vs[b], c, d);
    vs[b].edges ~= Edge(vs[a], c, d);
  }
  ...
  auto tree = redBlackTree!(
    (a, b) => a.t != b.t ? a.t < b.t : a.index < b.index,
    Vertex,
  )(vs[0]);
  ...
  while(!tree.empty) {
    ...
    foreach(e; v.edges) {
      if (e.end.t <= v.t) continue;

      long f(long x) {
        return e.c + (e.d / (v.t + x + 1)) + x;
      }


      long l = 0;
      long r = max(1, e.d - v.t - 1 + 5);
      ...
      if (e.end.t <= v.t + minV) continue;
      tree.removeKey(e.end);
      e.end.t = v.t + minV;
      tree.insert(e.end);
    }
  }
