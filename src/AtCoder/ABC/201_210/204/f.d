...
void main() {
  ...
  ModNum[][] mss = new ModNum[][](N, N);
  foreach(bits; 0..N) foreach(next; 0..N) {
    if (bits & next) continue;
    long res = 0;
    foreach(cur; 0..N) {
      bool ok = true;
      if (cur>>0&1) {
      ...
      foreach(i; 0..H) {
        if ((i<H-1) && ((cur>>(i+1)&1)&((bits>>i&1)|(next>>i&1)|(bits>>(i+1)&1)|(next>>(i+1)&1)))) {
          ok = false;
        }
        if ((i>0) && ((cur>>(i)&1)&((bits>>i&1)|(next>>i&1)|(bits>>(i-1)&1)|(next>>(i-1)&1)))) {
          ok = false;
        }
      }
      if (ok) {
  ...
  ModNum[][] mul(ModNum[][] xss, ModNum[][] yss) {
    ModNum[][] zss = new ModNum[][](N, N);
    foreach(i; 0..N) foreach(j; 0..N) {
      foreach(k; 0..N) {
        zss[i][j] += xss[i][k]*yss[k][j];
      }
    }
    return zss;
  }
  ...
  ModNum[][] pow(ModNum[][] base, long power) {
    ModNum[][] result = id();
    for (; power > 0; power >>= 1) {
      if (power & 1) {
  ...
  ModNum[][] pss = pow(mss, max(0, W-2));
  ModNum[] xs = mss[0];
  ModNum[] ys = N.iota.map!(i => mss[i][0]).array;
