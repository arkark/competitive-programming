...
void main() {
  ...
  long[][] vss = new long[][](R, C);
  foreach(i; 0..R) vss[i][] = -1;
  foreach(i; 0..K) {
    long r, c, v;
    scanln(r, c, v);
    r--; c--;
  ...
  long M = 3;
  long[][][] dp = new long[][][](R, C, M+1);
  foreach(i; 0..R) foreach(j; 0..C) dp[i][j][] = -INF;
  dp[0][0][0] = 0;

  foreach(i; 0..R) foreach(j; 0..C) {
    if (vss[i][j] > 0) {
      foreach_reverse(k; 0..M) {
        if (dp[i][j][k] < 0) continue;
        dp[i][j][k+1].ch!max(dp[i][j][k] + vss[i][j]);
      }
    }
    foreach(k; 0..M+1) {
      if (dp[i][j][k] < 0) continue;
      if (i < R-1) dp[i+1][j][0].ch!max(dp[i][j][k]);
      if (j < C-1) dp[i][j+1][k].ch!max(dp[i][j][k]);
    }
  }
  dp[R-1][C-1].reduce!max.writeln;
}
