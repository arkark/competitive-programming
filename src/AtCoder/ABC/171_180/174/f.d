...
void main() {
  long N, Q;
  scanln(N, Q);
  long[] cs = readln.split.to!(long[]).map!"a-1".array;
  ...
  foreach(i; 0..N) {
    if (ds[cs[i]] != -1) {
      seg.update(ds[cs[i]], seg.get(ds[cs[i]]) - 1);
    }
    ds[cs[i]] = i;
    seg.update(ds[cs[i]], seg.get(ds[cs[i]]) + 1);

    while(j < Q && ps[j].r == i+1) {
      ps[j].value = seg.query(0, ps[j].r) - seg.query(0, ps[j].l);
      j++;
    }
  }
  assert(j == Q);

  ps.sort!"a.index < b.index";
  foreach(p; ps) {
    p.value.writeln;
  }
