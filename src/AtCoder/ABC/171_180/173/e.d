...
void main() {
  ...
  if (K == N) {
  ...
  } else if (xs.empty && K % 2 != 0) {
    foreach_reverse (y; ys[$ - K .. $]) {
      ans *= y;
    }
    ans *= -1;
  } else {
    ...
    long i = 0, j = 0;
    while (i + j < K) {
      if (i + j + 1 < K && i + 1 < xs.length && j + 1 < ys.length) {
        long x = xs[i] * xs[i + 1];
        long y = ys[j] * ys[j + 1];
        if (x > y) {
          ans *= xs[i];
...
struct ModNumber(T, T mod) if (__traits(isIntegral, T)) {
  ...
  ModNumber opBinary(string op)(T that) if (op == "+" || op == "-" || op == "*") {
    return mixin("this " ~ op ~ " ModNumber(that)");
  }

  ModNumber opBinaryRight(string op)(T that) if (op == "+" || op == "-" || op == "*") {
    return mixin("ModNumber(that) " ~ op ~ " this");
  }
