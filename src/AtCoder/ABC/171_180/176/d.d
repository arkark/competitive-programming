...
void main() {
  ...
  foreach (i; 0 .. H)
    foreach (j; 0 .. W) {
      yss[i][j] = xss[i][j] ? -1 : INF;
    }
  bool[][] zss = new bool[][](H, W);
  ...
  while (!list.empty) {
    ...
    foreach (k; 0 .. 4) {
      P q = P(p.h + dh[k], p.w + dw[k]);
      if (q.h < 0 || q.h >= H)
        continue;
      if (q.w < 0 || q.w >= W)
        continue;
      if (xss[q.h][q.w] || zss[q.h][q.w])
        continue;

      long d = yss[p.h][p.w] + 0;
      if (d < yss[q.h][q.w]) {
        yss[q.h][q.w] = d;
        list.insertFront(q);
      }
  ...
  writeln(yss[g.h][g.w] == INF ? -1 : yss[g.h][g.w]);
}
