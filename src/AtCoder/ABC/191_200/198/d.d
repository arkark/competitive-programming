...
void main() {
  long K = 26;
  long[] xs = readln.chomp.map!(c => (c - 'a').to!long).array;
  long[] ys = readln.chomp.map!(c => (c - 'a').to!long).array;
  long[] zs = readln.chomp.map!(c => (c - 'a').to!long).array;
  ...
  foreach (perm; 10L.iota.permutations) {
    if (perm[us[xs[0]]] == 0)
      continue;
    if (perm[us[ys[0]]] == 0)
      continue;
