...
void main() {
  long N, M, K;
  scanln(N, M, K);
  ModNum[] as = readln.split
    .to!(long[])
    .map!(a => ModNum(a))
    .array;

  bool[][] ess = new bool[][](N, N);
  long[] ds = new long[N];
  foreach (i; 0 .. M) {
    long x, y;
    scanln(x, y);
  ...
  foreach (i; 0 .. N) {
    foreach (j; 0 .. N) {
      if (i == j) {
        mss[i][j] = (ModNum(M - ds[j]) + ModNum(ds[j]) / 2) / M;
      } else if (ess[i][j]) {
        mss[i][j] = ModNum(1) / 2 / M;
      } else {
        mss[i][j] = 0;
      }
    }
