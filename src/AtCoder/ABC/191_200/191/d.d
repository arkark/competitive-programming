...
void main() {
  ...
  long ce(double x) {
    return cast(long)(std.math.ceil(x) + 0.01);
  }
...

  {
    ...
    long f(string t) {
      string[] ts = t.split(".").array;
      if (ts.length == 1) {
        return ts[0].to!long * T;
      } else {
        return ts[0].to!long * T + ("1" ~ ts[1]).to!long * (
            10 ^^ (T.to!string.length - 1 - ts[1].length)) - T;
      }
    }
  ...
  long minY = fl((Y - R) / 10000.0);
  long maxY = ce((Y + R) / 10000.0);

  foreach (ty; minY .. maxY + 1) {
    long y = ty * 10000;

    long centerX = X;
    if (!inCircle(centerX, y)) {
      centerX = X - 1;
    }
    if (!inCircle(centerX, y)) {
      centerX = X + 1;
