...
void main() {
  ...
  foreach(i; 0..M) {
    ...
    ps[i].cs = readln.split.to!(long[]).map!"a-1".array;
  }

  auto dijkstra = Dijkstra!long(1L<<N);
  foreach(bits; 0..1L<<N) {
    foreach(p; ps) {
      long next = bits;
      foreach(c; p.cs) {
      ...
      dijkstra.addEdge(bits, next, p.a);
    }
  }
  dijkstra.solve(0, (1L<<N) - 1).back.pipe!"a==long.max?-1:a".writeln;
}
