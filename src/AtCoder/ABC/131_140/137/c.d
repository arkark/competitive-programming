...
void main() {
  long N;
  scanln(N);
  auto xs = N.rep!(() => readln.chomp.to!(char[]).map!(to!int).array.sort!"a<b".map!(to!char).array.to!string).array.sort!"a<b".array.group.array;
  long ans = 0;
  foreach(x; xs) {
    ans += x[1].to!long*(x[1].to!long - 1)/2;
  }
  ans.writeln;
}
