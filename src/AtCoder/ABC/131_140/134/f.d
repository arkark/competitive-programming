...
void main() {
  ...
  ModNum[][][] dp = new ModNum[][][](N+1, N+1, N*N+1);
  dp[0][0][0] = 1;

  bool ok(long i, long j, long k) {
    if (i<0 || i>N) return false;
    if (j<0 || j>N) return false;
    if (k<0 || k>N*N) return false;
    return true;
  }
  void add(ref ModNum x, long i, long j, long k, long t) {
    if (!ok(i, j, k)) return;
    x += t * dp[i][j][k];
  }
  foreach(i; 0..N) foreach(j; 0..N+1) foreach(k; 0..N*N+1) {
    add(dp[i+1][j][k], i, j-1, k-2*j, 1);
    add(dp[i+1][j][k], i, j, k-2*j, 2*j + 1);
    add(dp[i+1][j][k], i, j+1, k-2*j, (j + 1)*(j + 1));
  }
