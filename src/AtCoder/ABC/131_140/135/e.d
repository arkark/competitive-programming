...
Q[] solve(long K, Q gq, Q diff) {
  if (diff.u != 0 && (diff.u + K) % 2 != 0) return [];
  if (diff.v != 0 && (diff.v + K) % 2 != 0) return [];
  Q[] qs = [];
  Q q = Q(0, 0);
  if (diff.u != 0) {
    q.u += diff.u;
...
void main() {
  ...
  Q diff = Q((gq.u%K + K)%K, (gq.v%K + K)%K);
  Q[][] qss = [
    solve(K, gq, Q(diff.u, diff.v)),
    solve(K, gq, Q(diff.u - K, diff.v)),
    solve(K, gq, Q(diff.u, diff.v - K)),
    solve(K, gq, Q(diff.u - K, diff.v - K))
  ].filter!(qs => !qs.empty).array;
  if (qss.empty) {
    writeln(-1);
  } else {
    long m = qss.map!(qs => qs.length).reduce!min;
    foreach(qs; qss) {
      if (qs.length == m) {
        m.writeln;
        // gq.writeln;
