...
void main() {
  ...
  scanln(Q);
  bool rev = false;
  auto deq = Deque!char(s);
  foreach(i; 0..Q) {
    string[] qs = readln.split.array;
    long t() {return qs[0].to!long;}
    long f() {return qs[1].to!long;}
    char c() {return qs[2].to!char;}
    if (t == 1) {
      rev ^= true;
    } else {
      if (rev ^ (f == 2)) {
  ...
  if (rev) {
    array(deq).map!(to!char).retro.writeln;
  } else {
    array(deq).map!(to!char).writeln;
  }
}
