...
void main() {
  ...
  long N;
  scanln(N);
  long[] xs = readln.chomp.map!(c => to!long(c - 'a')).array;
  long Q;
  scanln(Q);
  ...
  foreach(_1; 0..Q) {
    string[] qs = readln.split.to!(string[]);
    if (qs[0] == "1") {
      long i = qs[1].to!long - 1;
      long c = to!long(qs[2].to!char - 'a');
      seg.update(i, 1L<<c);
    } else {
      long l = qs[1].to!long - 1;
      long r = qs[2].to!long;
      seg.query(l, r).popcnt.writeln;
    }
  }
}
