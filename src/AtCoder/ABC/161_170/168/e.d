...
void main() {
  ...
  foreach(b, v; aa) {
    foreach(a, num; v) {
      ...
      long x = num;
      long y = 0;
      if (a < 0 && -a in aa && b in aa[-a]) {
        y += aa[-a][b];
      } else if (a >= 0 && a in aa && -b in aa[a]) {
        y += aa[a][-b];
      }
      ans *= ModNum(2)^^x + ModNum(2)^^y - 1;
      aa[b][a] = 0;
      if (y > 0) (a < 0 ? aa[-a][b] : aa[a][-b]) = 0;
    }
  }
  ans--;
  ans += zeroNum;
