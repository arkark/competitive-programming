...
void main() {
  ...
  auto trees = T.rep!(() => redBlackTree!("a.a != b.a ? a.a > b.a : a.i < b.i", true, P)());
  foreach(i; 0..N) {
    long a, b;
    scanln(a, b);
    b--;
  ...
  auto seg = SegTree!(long, min, INF)(T);
  foreach(i; 0..T) {
    if (trees[i].empty) continue;
    seg.update(i, trees[i].front.a);
  }
  ...
  foreach(i; 0..Q) {
    ...
    scanln(c, d);
    c--; d--;
    trees[ps[c].j].removeKey(ps[c]);
    if (trees[ps[c].j].empty) {
      seg.update(ps[c].j, INF);
    } else {
      seg.update(ps[c].j, trees[ps[c].j].front.a);
    }
    ps[c].j = d;
    trees[ps[c].j].insert(ps[c]);
    seg.update(ps[c].j, trees[ps[c].j].front.a);

    seg.query(0, T).writeln;
  }
}
