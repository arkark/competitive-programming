...
void main() {
  ...
  P s, g;
  scanln(s.y, s.x, g.y, g.x);
  s.x--; s.y--;
  g.x--; g.y--;

  bool[][] bss = H.rep!(() => readln.chomp.map!"a=='@'".array);
  ...
  while(!list.empty) {
    P p = list.front;
    list.removeFront;
    long d = dss[p.y][p.x] + 1;
    {
      // right
      foreach(i; 1..K+1) {
        P q = P(p.y, p.x + i);
        if (q.y < 0 || q.y >= H) break;
        if (q.x < 0 || q.x >= W) break;
        if (bss[q.y][q.x]) break;
        if (d > dss[q.y][q.x]) break;
        if (d < dss[q.y][q.x]) {
          list.insertBack(q);
          dss[q.y][q.x] = d;
        }
