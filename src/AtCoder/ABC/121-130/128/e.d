...
void main() {
  ...
  P[] ps = new P[N];
  foreach(i; 0..N) {
    scanln(ps[i].s, ps[i].t, ps[i].x);
    ps[i].s -= ps[i].x;
    ps[i].t -= ps[i].x;
  }
  ps.sort!"a.x < b.x"; //
  long[] ds = Q.rep!(()=>readln.chomp.to!long);

  long[long] aa;
  long[] bb = [];
  {
    long[] ss = ps.map!"a.s".array;
    long[] ts = ps.map!"a.t".array;
    long[] xs = (ss ~ ts ~ ds).sort!"a<b".array.uniq.array;
    foreach(long i, x; xs) {
      aa[x] = i;
      bb ~= x;
    }
  }
  long L = bb.length.to!long;

  long[] bs = new long[L+1];
  long[][] ess = new long[][L+1];
  long[][] fss = new long[][L+1];
  foreach(i; 0..N) {
    bs[aa[ps[i].s]]++;
    bs[aa[ps[i].t]]--;
    ess[aa[ps[i].s]] ~= ps[i].x;
    fss[aa[ps[i].t]] ~= ps[i].x;
  }
  foreach(i; 0..L) {
    bs[i+1] += bs[i];
  }
  long[] cs = new long[L+1];
  auto tree = redBlackTree!("a < b", true, long)();
  foreach(i; 0..L+1) {
    foreach(e; ess[i]) tree.insert(e);
    foreach(f; fss[i]) tree.removeKey(f);
    if (tree.empty) {
      cs[i] = -1;
    } else {
      cs[i] = tree.front;
