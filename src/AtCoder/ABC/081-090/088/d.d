...
void main() {
    int H, W;
    scanln(H, W);
    bool[][] grid = H.rep!(() => readln.chomp.map!"a=='#'".array);

    class Node {
        int y, x;
        long d = INF;
        bool used;
        this(int y, int x, bool used) {
            this.y = y;
            this.x = x;
            this.used = used;
        }
    }
    Node[][] nodes = H.iota.map!(
        y => W.iota.map!(
            x => new Node(y, x, grid[y][x])
        ).array
    ).array;
    auto list = DList!Node(nodes[0][0]);
    nodes[0][0].used = true;
    nodes[0][0].d = 1;
    ...
    if (!nodes[H-1][W-1].used) {
        writeln(-1);
        return;
    }

    writeln(H*W - grid.map!(as => as.count!"a").sum - nodes[H-1][W-1].d);
