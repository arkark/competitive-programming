...
void main() {
  long A, B, Q;
  scanln(A, B, Q);
  auto ss = (A.rep!(() => readln.chomp.to!long) ~ [-INF, INF]).sort!"a<b".array.assumeSorted!"a<b";
  auto ts = (B.rep!(() => readln.chomp.to!long) ~ [-INF, INF]).sort!"a<b".array.assumeSorted!"a<b";
  foreach(_; 0..Q) {
    long x = readln.chomp.to!long;
    long[] ps = [
      ss.lowerBound(x+1).back,
      ss.upperBound(x-1).front,
      ts.lowerBound(x+1).back,
      ts.upperBound(x-1).front
    ];
    long ans = INF;
    long[] perm = 4L.iota.array;
    do {
      long res = 0;
      bool[] bs = [false, false];
      long y = x;
