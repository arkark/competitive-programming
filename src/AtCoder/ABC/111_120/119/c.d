...
void main() {
  ...
  long K = 3;
  long[] as = [A, B, C];
  long[] ls = N.rep!(()=>readln.chomp.to!long);
  long ans = INF;
  foreach(digits; 0..4^^N) {
    long[] xs = N.iota.map!(i => digits>>(i<<1)&0b11).array;
    long[] bs = K.iota.map!(i => xs.count(i).to!long).array;
    long[] cs = K.iota.map!(
      i => N.iota.filter!(j => xs[j]==i).map!(j => ls[j]).sum
    ).array;
    if (bs.reduce!"a*b" == 0) continue;

    ans.ch!min(
      zip(as, bs, cs).map!(
        t => (t[1]-1)*10 + abs(t[0] - t[2])
      ).sum
    );
  }
  ans.writeln;
