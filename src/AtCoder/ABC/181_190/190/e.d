...
void main() {
  ...
  long f(Vertex s, Vertex g) {
    foreach (v; vs) {
      v.used = false;
    }

    s.used = true;
    auto list = DList!P(P(s, 0));
    while (!list.empty) {
      P p = list.front;
      list.removeFront;
      assert(p.v.used);
      ...
      foreach (u; p.v.adj) {
        ...
        u.used = true;
        list.insertBack(P(u, p.d + 1));
      }
    }
    return INF;
  }

  long[][] dss = new long[][](K, K);
  foreach (i; 0 .. K) {
    foreach (j; i + 1 .. K) {
      dss[i][j] = f(vs[cs[i]], vs[cs[j]]);
    }
  }
    ...
    foreach (i; 0 .. K) {
      ...
      foreach (j; 0 .. K) {
        if (bits >> j & 1)
          continue;
        long next = bits | (1 << j);
        long d = dss[min(i, j)][max(i, j)];
        dp[next][j].ch!min(dp[bits][i] + d);
      }
    }
  }

  long ans = dp[(1 << K) - 1].reduce!min;
  if (ans >= INF) {
    writeln(-1);
  } else {
    writeln(ans + 1);
