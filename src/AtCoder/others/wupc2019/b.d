...
void main() {
  ...
  long[][] vss;
  {
    long[][] uss = H.rep!(()=>readln.split.to!(long[]));
    if (H<=W) {
      vss = uss;
    } else {
      swap(H, W);
  ...
  if (vss.all!(all!"a==0")) {
    writeln("Yes", " ", 0);
  } else if (vss.all!(all!"a==5")) {
    writeln("Yes", " ", 1);
  } else if (vss.all!(all!"a!=5")) {
    writeln("No");
  } else if (vss.length == 1) {
    long[] vs = vss[0];
    long[] as = vs.cumulativeFold!max(0L).array;
    long[] bs = vs.retro.cumulativeFold!max(0L).array.retro.array;
    long c = INF;
    foreach(i, v; vs) {
      if (v != 5) continue;
      c.ch!min((i==0 ? 0 : ts[as[i-1]]) + (i==W-1 ? 0 : ts[bs[i+1]]));
    }
    writeln("Yes", " ", c + 1);
  } else {
    long m = vss.map!(
      vs => vs.filter!"a!=5".array
    ).filter!(
      vs => !vs.empty
    ).map!(
      vs => vs.reduce!max
