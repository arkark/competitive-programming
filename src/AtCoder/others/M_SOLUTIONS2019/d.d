...
void main() {
  ...
  vs.sort!"a.edges.length > b.edges.length";

  auto cs = DList!long(readln.split.to!(long[]).sort!"a>b".array);

  Vertex s = vs.front;

  auto tree = redBlackTree!(
    "a.edges.length != b.edges.length ? a.edges.length > b.edges.length : a.index < b.index",
    true,
    Vertex
  )(s);
  ...
  while(!tree.empty) {
    ...
    foreach(e; v.edges) {
      if (e.t.visited) continue;
      tree.insert(e.t);
    }
  }
  ans.writeln;
  vs.sort!"a.index<b.index".map!(v => v.c.to!string).join(" ").writeln;
}
