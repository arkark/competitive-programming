...
void main() {
  long N, M;
  scanln(N, M);
  long[] as = readln.split.to!(long[]).map!"a-1".array.sort!"a<b".array;
  long[] bs = readln.split.to!(long[]).map!"a-1".array.sort!"a<b".array;

  if (as.uniq.count < as.length) {
    0.writeln;
    return;
  }
  if (bs.uniq.count < bs.length) {
    0.writeln;
    return;
  }
  ...
  foreach_reverse(ref p; ps) {
    ...
    if (p.i != -1 && p.j != -1) {
      ...
      ans *= (M-p.y) - max(0, num - (N-x-1)*(M-y));
    } else if (p.j != -1) {
      ans *= (N-p.x) - max(0, (num - (N-x)*(M-y-1)));
    } else {
      ans *= (N-p.x)*(M-p.y) - num;
    }
    num++;
