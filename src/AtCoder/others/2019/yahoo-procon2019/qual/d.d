...
void main() {
  long L;
  scanln(L);
  long[] as = L.rep!(() => readln.chomp.to!long);
  ...
  long[] ds = new long[L+1];
  foreach(i; 0..L) {
    ds[i+1] = ds[i] + (as[i]+1)%2;
  }

  auto seg = RMQ!long(
    (L+1).iota.map!(j => ds[j] + rs[j]).array
  );

  long ans = INF;
  foreach(i; 0..L) {
    long v = ls[i] - ds[i] + seg.query(i, L+1);
    ans.ch!min(v);
  }
  ans.writeln;
}
