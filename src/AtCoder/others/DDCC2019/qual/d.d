...
void main() {
  long K = 29;
  long[] as = K.rep!(() => readln.chomp.to!long);
  ...
  auto result = chineseRemainderTheorem(1L.repeat(K).array, bs, ms);
  if (result.x < 0 || result.x > 10L^^12) {
    "invalid".writeln;
    return;
  }
...
// @return: a^-1 mod m
// @assert: gcd(a, m) = 1
long inverseMod(long a, long m) in {
  assert(gcd(a, m) == 1);
} body {
  long x, y;
  extgcd(a, m, x, y);
...
Tuple!(long, "x", long, "m") chineseRemainderTheorem(long[] as, long[] bs, long[] ms) in {
