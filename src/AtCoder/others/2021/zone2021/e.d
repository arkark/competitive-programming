...
void main() {
  ...
  auto solver = Dijkstra!long(R * C * 2);
  long[][] ass = R.rep!(() => readln.split.to!(long[]));
  long[][] bss = R.rep!(() => readln.split.to!(long[]));

  long f(long i, long j, long k = 0) {
    return k * R * C + i * C + j;
  }
  ...
  foreach (i; 0 .. R)
    foreach (j; 0 .. C) {
      if (j > 0) {
        solver.addEdge(f(i, j), f(i, j - 1), ass[i][j - 1]);
      }
      if (j < C - 1) {
        solver.addEdge(f(i, j), f(i, j + 1), ass[i][j]);
      }
      if (i < R - 1) {
        solver.addEdge(f(i, j), f(i + 1, j), bss[i][j]);
      }
      solver.addEdge(f(i, j), f(i, j, 1), 1);
      solver.addEdge(f(i, j, 1), f(i, j), 0);
      if (i > 0) {
        solver.addEdge(f(i, j, 1), f(i - 1, j, 1), 1);
      }
    }

  writeln(solver.solve(f(0, 0))[f(R - 1, C - 1)]);
}
