...
void main() {
  ...
  foreach (_; 0 .. K) {
    ...
    h--;
    w--;
    xss[h][w] = c == 'X' ? 1 : c == 'R' ? 2 : c == 'D' ? 3 : 0;
  }

  ModNum div23 = ModNum(2) / 3;

  ModNum[][] dp = new ModNum[][](H + 1, W + 1);
  dp[0][0] = ModNum(3) ^^ (H * W - K);
  foreach (i; 0 .. H) {
    foreach (j; 0 .. W) {
      if (xss[i][j] > 0) {
        if (xss[i][j] == 1 || xss[i][j] == 2) {
          dp[i][j + 1] += dp[i][j];
        }
        if (xss[i][j] == 1 || xss[i][j] == 3) {
          dp[i + 1][j] += dp[i][j];
        }
      } else {
        auto t = dp[i][j] * div23;
