...
void main() {
  ...
  foreach(i; 0..N) {
    foreach(j; 0..N) {
      if (j%5 == i*2%5) grid[i][j] = true;
    }
  }
  foreach(i; 0..N) {
    if (i+2<N && grid[0][i+2]) grid[0][i] = true;
    if (i+3<N && grid[0][i+3]) grid[0][i] = true;
    if (i+2<N && grid[i+2][0]) grid[i][0] = true;
    if (i+3<N && grid[i+3][0]) grid[i][0] = true;
    if (i+2<N && grid[N-1][i+2]) grid[N-1][i] = true;
    if (i+3<N && grid[N-1][i+3]) grid[N-1][i] = true;
    if (i+2<N && grid[i+2][N-1]) grid[i][N-1] = true;
    if (i+3<N && grid[i+3][N-1]) grid[i][N-1] = true;
  }
  grid[0][1] = grid[1][0] = grid[N-1][1] = grid[N-2][0] = grid[1][N-1] = grid[N-2][0] = grid[N-2][N-1] =  grid[N-1][N-2] = true;
  grid.each!(
    xs => xs.map!(a => a?'X':'.').writeln
  );
}
