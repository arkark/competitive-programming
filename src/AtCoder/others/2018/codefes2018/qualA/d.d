...
alias Pair = Tuple!(ulong, "index", long, "value");

void main() {
  long D, F, T, N;
  scanln(D, F, T, N);
  auto xs = ([0L] ~ readln.split.to!(long[]) ~ [D]).enumerate.array;

  ModNum[] ps = new ModNum[N+1];
  ps[0] = 1;
  foreach(i; 0..N) {
    ps[i+1] = ps[i]*2;
  }

  ModNum[] dp = new ModNum[N+2];
  ModNum[] imos = new ModNum[N+3];
  dp[0] = 1;
  foreach(i; 0..N+1) {
    if (i > 0) imos[i] += imos[i-1];
    dp[i] += imos[i];

    auto reL = xs.assumeSorted!"a.value < b.value".upperBound(
      Pair(-1, xs[i].value + (F-T))
    );
    auto reR = xs.assumeSorted!"a.value < b.value".lowerBound(
      Pair(-1, xs[i].value + F + 1)
    );
    if (reL.empty || reR.empty) continue;

    long l = reL.front.index;
    long r = reR.back.index;
    ModNum v = dp[i]*ps[l-i-1];
    imos[l] += v;
    imos[r+1] -= v;
  }
  imos[N+1] += imos[N];
