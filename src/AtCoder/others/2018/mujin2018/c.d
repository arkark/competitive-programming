...
void main() {
  ...
  foreach(i; 0..N) {
    foreach(j; 0..M) {
      ...
      if (isSpace(i, j-1)) dp1[i][j] += dp1[i][j-1] + 1;
    }
    foreach_reverse(j; 0..M) {
      if (!isSpace(i, j)) continue;
      if (isSpace(i, j+1)) dp2[i][j] += dp2[i][j+1] + 1;
    }
  }
  foreach(j; 0..M) {
    foreach(i; 0..N) {
      if (!isSpace(i, j)) continue;
      if (isSpace(i-1, j)) dp3[i][j] += dp3[i-1][j] + 1;
    }
    foreach_reverse(i; 0..N) {
      if (!isSpace(i, j)) continue;
      if (isSpace(i+1, j)) dp4[i][j] += dp4[i+1][j] + 1;
    }
  }
