...
void main() {
  ...
  auto uf = UnionFind((K+1)*(K+1));
  foreach(i; 1..K+1) foreach(j; 1..K+1) {
    P p = P(i, j);
    P q = p;
    with(q) {
      if (x<y) {
        x = x.to!string.retro.to!string.to!long;
      } else {
        y = y.to!string.retro.to!string.to!long;
      }
      if (x<y) {
        y = y-x;
      } else {
    ...
    uf.unite(p.x*(K+1)+p.y, q.x*(K+1)+q.y);
  }

  long ans = 0;
  foreach(i; 1..N+1) foreach(j; 1..M+1) {
    if (!uf.same(0, i*(1000+1)+j)) ans++;
  }
  ans.writeln;
}
