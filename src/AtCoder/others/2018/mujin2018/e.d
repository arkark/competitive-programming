...
void main() {
  long N, M, K;
  scanln(N, M, K);
  long[] ds = readln.chomp.map!(
    a => "UDLR".countUntil(a).to!long
  ).array;
  bool[][] bss = new bool[][](N, M);
  P start, goal;
  foreach(i; 0..N) {
  ...
  bool isSpace(long i, long j) {
    if (i<0 || i>=N) return false;
    if (j<0 || j>=M) return false;
    if (bss[i][j]) return false;
    return true;
  }

  long[][] nexts = new long[][](K, 4);
  foreach(i; 0..K) nexts[i][] = INF;
  foreach(_; 0..2) {
    foreach_reverse(i; 0..K) {
      nexts[i][ds[i]] = 1;
      foreach(j; 0..4) {
        nexts[i][j].ch!min(nexts[(i+1)%$][j] + 1);
      }
    }
  }
  ...
  while(!tree.empty) {
    ...
    if (visited[t.pos.y][t.pos.x]) continue;
    visited[t.pos.y][t.pos.x] = true;

    foreach(i; 0..4) {
      long dt = nexts[t.time%K][i];
      if (dt == INF) continue;
      P p = P(
        t.pos.y + dy[i],
        t.pos.x + dx[i]
      );
      if (!isSpace(p.y, p.x)) continue;
      if (visited[p.y][p.x]) continue;
      tree.insert(T(p, t.time + dt));
    }
  }
  writeln(-1);
}
