...
void main() {
  ...
  ModNum ans = ModNum(2)^^((N*N - N)/2 - visited.length);

  Vertex[] vs = N.iota.map!(i => new Vertex(i)).array;
  foreach(k, c; aa) {
    if (k.b - k.a == 0) {
      if (vs[k.a].c>=0 && vs[k.a].c != c) {
        0.writeln;
        return;
      }
      vs[k.a].c = c;
    } else if (k.b - k.a == 1) {
      vs[k.a].edge = new Edge(vs[k.a], vs[k.b], c);
    } else if (k.b - k.a == 2) {
      if (vs[k.b-1].c>=0 && vs[k.b-1].c != c) {
        0.writeln;
        return;
      }
      vs[k.b-1].c = c;
  ...
  foreach(v; us) {
    ...
    long g(Vertex u, long c) {
      if (u.c>=0 && u.c != c) return 0;

      if (u.edge is null) {
        return 1;
      } else {
        return g(u.edge.end, (c + u.edge.c)%2);
      }
    }
