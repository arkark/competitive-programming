...
void main() {
  long N;
  scanln(N);
  bool[][] ass = N.rep!(() => readln.chomp.map!"a=='1'".array);
  bool[][] bss = N.rep!(() => readln.chomp.map!"a=='1'".array);

  bool[][] xss = new bool[][](N, N);
  bool[][] yss = new bool[][](N, N);
  foreach(i, as; ass) {
    foreach(j, a; as) {
      xss[j][i] = a;
    }
  ...
  bool[][] css = new bool[][](N, N);

  bool[][] zss = new bool[][](N, N);
  foreach(i; 0..N) {
    foreach(j; 0..N) {
      bool ok = true;
      foreach(k, x; xss[i]) {
