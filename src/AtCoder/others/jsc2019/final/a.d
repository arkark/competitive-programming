...
void main() {
  ...
  auto bs = readln.split.to!(long[]).enumerate.array.sort!"a.value<b.value".array;

  long T = 1000000;
  long[] cs = new long[T+1];
  long[] ds = new long[T+1];
  foreach(a; as) cs[a.value]++;
  foreach(b; bs) ds[b.value]++;

  long[] es = Ntt.mul(cs, ds, T+1);
  auto po = es.enumerate.find!"a.value>=2";
  if (po.empty) {
    writeln(-1);
  } else {
    long x = po.front.index;
    long[] ans = [];
    foreach(a; as) {
      if (ans.length == 4) break;
      long l = -1;
      long r = M;
      while(r - l > 1) {
        long c = (l + r)/2;
        if (bs[c].value <= x - a.value) {
          l = c;
        } else {
          r = c;
        }
      }
      if (l >= 0 && bs[l].value == x-a.value) {
        ans ~= [a.index, bs[l].index];
      }
    }
    assert(ans.length == 4);
