...
Edge[] f(long n, long level) {
  Edge[] edges = [];
  foreach(i; 0..n/2) {
    foreach(j; n/2..n) {
      edges ~= Edge(i, j, level);
    }
  }
  return edges;
}

Edge[] g(long n, long level) {
  if (n < 2) return [];
  Edge[] edges = f(n, level);
  edges ~= g(n/2, level + 1);
  edges ~= g(n-n/2, level + 1).map!(
    e => Edge(e.a + n/2, e.b + n/2, e.level)
  ).array;
  return edges;
}
...
void main() {
  ...
  Edge[] edges = g(N, 1);
  long[][] ans = (N-1).iota.map!(i => new long[N-i-1]).array;
  foreach(e; edges) {
    long a = e.a;
    long b = e.b;
    if (a > b) swap(a, b);
  ...
  ans.each!(
    xs => xs.map!(to!string).join(" ").writeln
  );
}
