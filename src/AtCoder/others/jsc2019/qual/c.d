...
void main() {
  long N;
  scanln(N);
  long[] xs = readln.chomp.map!"a=='W'?1:0".map!(to!long).array;
  ModNum[long][] dp = new ModNum[long][2*N+1];
  dp[0][0] = 1;
  foreach(i; 0..2*N) {
    foreach(j; dp[i].keys) {
      if ((j+xs[i])%2 == 1) {
        if (j-1 !in dp[i+1]) dp[i+1][j-1] = ModNum(0);
        dp[i+1][j-1] += dp[i][j] * j;
      }
      if ((j+xs[i])%2 == 0) {
        if (j+1 !in dp[i+1]) dp[i+1][j+1] = ModNum(0);
  ...
  if (0 in dp[2*N]) {
    writeln(dp[2*N][0] * ModNum.fact(N));
  } else {
    0.writeln;
  }
}
