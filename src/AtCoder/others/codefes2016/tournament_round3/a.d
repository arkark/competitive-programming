...
void main() {
  ...
  foreach(j; 1..K+1) {
    dp[j%2][] = -INF;
    long[] xs = dp[(j-1)%2][0..N];
    size_t[] ys = xs.slideMinimum!(long, "a>b")(M);
    foreach(i; 0..N) {
      dp[j%2][i+1] = xs[ys[i]] + j*as[i];
    }
  }
  dp[K%2].reduce!max.writeln;
}
...
if(is(typeof(binaryFun!fun(T.init, T.init)) == bool)) in {
  assert(k > 0);
} body {
  size_t n = xs.length;
  size_t[] deq = new size_t[n];
  size_t l = 0, r = 0;
  size_t empty() { return l == r; }
  size_t front() { return deq[l]; }
  size_t back() { return deq[r-1]; }
  void removeFront() { l++; }
  void removeBack() { r--; }
  void insertBack(size_t v) { deq[r++] = v; }
