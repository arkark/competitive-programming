...
void main() {
  ...
  long[] bs = [0L] ~ as.cumulativeFold!"a+b"(0L).array;

  long[] cs = new long[N*(N+1)/2];
  {
    long i = 0;
    foreach(r; 0..N) foreach(l; 0..r+1) {
      cs[i] = bs[r+1] - bs[l];
      i++;
    }
  }
  ...
  long M = 41;
  long ans = 0;
  void f(long depth, long s, bool[] using) {
    if (depth<0) {
      ans.ch!max(s);
    } else {
      bool[] _using = using.dup;
      long c = 0;
      foreach(i, u; using) {
        if (!u) continue;
        if (cs[i]>>depth&1) {
      ...
      if (c >= K) {
        f(depth - 1, s + (1L<<depth), _using);
      } else {
        f(depth - 1, s, using);
      }
    }
  }
  f(M, 0, true.repeat(cs.length).array);
