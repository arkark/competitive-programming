...
void main() {
  long N;
  scanln(N);
  ModNum[] as = readln.split.to!(long[]).map!(a => ModNum(a)).array;
  ModNum[] bs = new ModNum[N+1];
  foreach(i; 0..N) {
    bs[i+1] = bs[i] + as[i];
  }

  ModNum[] xs = N.iota.map!"a+1".map!(a => 1/ModNum(a)).array;
  foreach(i; 1..N) {
    xs[i] += xs[i-1];
  }
