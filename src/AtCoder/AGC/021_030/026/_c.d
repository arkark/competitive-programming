...
void main() {
  ...
  long[string[2]][] xss = new long[string[2]][N+1];
  foreach(bits; 0..1L<<N) {
    string s="";
    string t="";
    foreach(i; 0..N) {
    ...
    xss[bits.popcnt][[s, t.retro.array.to!string]]++;
  }

  long[string[2]][] yss = new long[string[2]][N+1];
  foreach(bits; 0..1L<<N) {
    string s="";
    string t="";
    foreach(i; 0..N) {
    ...
    yss[bits.popcnt][[s, t.retro.array.to!string]]++;
  }
  ...
  foreach(i, xs; xss) {
    foreach(k, v; xs) {
      string[2] z = [k[1], k[0]];
      if (auto p = z in yss[N-i]) ans += v * *p;
    }
  }
  ans.writeln;
}
