...
void main() {
  ...
  foreach(bits; 0..1L<<N) {
    ...
    xss[s.length] ~= [s, t.retro.array.to!string];
  }

  string[2][][] yss = new string[2][][N+1];
  foreach(bits; 0..1L<<N) {
    string s="";
    string t="";
    foreach(i; 0..N) {
    ...
    yss[s.length] ~= [s, t.retro.array.to!string];
  }

  long ans = 0;
  foreach(i; 0..N+1) {
    auto fun = (string[2] a, string[2] b) => a[0]!=b[0] ? a[0]<b[0] : a[1]<b[1];
    auto eq = (string[2] a, string[2] b) => a[0]==b[0] && a[1]==b[1];
    string[2][] xs = xss[i].sort!((a, b) => fun(a, b)).array;
    string[2][] ys = yss[N-i].sort!((a, b) => fun(a, b)).array;
    // i.writeln;
    // xs.writeln;
    // ys.writeln;
    ...
    foreach(x; xs) {
      ...
      long Y = ys.length;
      string[2] p = [x[1], x[0]];
      if (!ys.assumeSorted!((a, b) => fun(a, b)).contains(p)) {
        continue;
      }

      // ans += ys.assumeSorted!((a, b) => fun(a, b)).equalRange(p).count; // too slow

      long l1 = 0;
      long r1 = Y;
      while(r1-l1>1) {
        long c = (l1+r1)/2;
        if (fun(p, ys[c]) || eq(p, ys[c])) {
          r1 = c;
        } else {
          l1 = c;
        }
