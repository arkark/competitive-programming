...
void main() {
    ...
    auto po = (iota('a', 'z').array ~ "z").find!(c => !str.canFind(c));
    if (po.empty) {
        int[] as = str.map!(c => c.to!int).array;
        if (as.nextPermutation) {
            string bs = as.map!(c => c.to!char).array.to!string;
