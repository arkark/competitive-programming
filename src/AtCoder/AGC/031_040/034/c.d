...
void main() {
  ...
  foreach(i; 0..N) {
    ps[i].index = i;
    scanln(ps[i].b, ps[i].l, ps[i].u);
  }

  P[] qs = ps.dup.sort!(
    (a, b) => a.g(X) - a.g(0) > b.g(X) - b.g(0)
  ).array;
  ...
  long right = N*X;
  while(right - left > 1) {
    long center = (left + right).ceil(2);
    long k = center.floor(X);
    long x = center - k*X;
    ...
    foreach(i; 0..N) {
      P p = ps[i];
      if (aa[i] < k) {
        maxD.ch!max(ds[k+1] + es[k+1] - p.g(X) + p.g(x));
      } else {
        maxD.ch!max(ds[k] + es[k] - p.g(0) + p.g(x));
      }
    }
    if (maxD >= 0) {
      right = center;
