...
void main() {
  ...
  long[] f(long dim, long a, long b) {
    assert(popcnt(a^b)%2 == 1);
    if (dim == 1) {
      return [a, b];
    } else {
      foreach(i; 0..dim) {
        if ((a>>i&1)^(b>>i&1)) {
          long a1 = (a>>(i+1)<<i)|(a&((1<<i)-1));
          long b1 = (b>>(i+1)<<i)|(b&((1<<i)-1));
          long c = a1 ^ (1<<(i==dim-1 ? i-1 : i));
          assert(c != a1 && c != b1);
          long[] ps = f(dim - 1, a1, c);
          long[] qs = f(dim - 1, c, b1);
          return ps.map!(
            p => (p>>i<<(i+1)) | (a&1<<i) | (p&((1<<i)-1))
          ).array ~ qs.map!(
            q => (q>>i<<(i+1)) | (b&1<<i) | (q&((1<<i)-1))
          ).array;
        }
      }
      assert(false);
  ...
  f(N, A, B).map!(to!string).join(" ").writeln;
}
