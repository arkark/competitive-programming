...
void main() {
  ...
  auto seg = LazySegTree!(
    Pair, long,
    (a, b) => Pair(a.num + b.num, a.a + b.a),
    "a+b",
    (a, b) => Pair(a.num, a.a + b*a.num),
    Pair(0, 0),
    0
  )(Pair(0, 0).repeat(N).array);

  auto bs = DList!(Tuple!(ulong, "index", long, "value"))(as.enumerate.array.sort!"a.value > b.value".array);
  ...
  foreach(i; 0..Q) {
    while(!bs.empty) {
      ...
      if (b.value > ps[i].x) {
        bs.removeFront;
        seg.set(b.index, Pair(1, b.value));
      } else {
        break;
      }
    }
    seg.update(ps[i].l, ps[i].r+1, -ps[i].x);
    ys[ps[i].i] = seg.query(ps[i].l, ps[i].r+1).a;
    seg.update(ps[i].l, ps[i].r+1, ps[i].x);
  }
  ys.each!writeln;
}
