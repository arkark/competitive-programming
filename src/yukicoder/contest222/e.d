...
void main() {
  ...
  auto seg = LazySegTree!(
    ...
    (a, b) {
      if (b.mod) {
        return R(
          b.mod,
          a.inverse ^ (a.afterDiff&1) ^ b.inverse,
          b.afterDiff
        );
      } else {
        assert(!b.mod);
    ...
    (a, b) {
      ...
      if (b.mod) {
        if (b.inverse) {
          swap(p.evenNum, p.oddNum);
          swap(p.evenSum, p.oddSum);
        }
        p.evenSum = 0;
        p.oddSum = p.oddNum;
      }
      p.evenSum += a.evenNum*b.afterDiff;
      p.oddSum += a.oddNum*b.afterDiff;
      if (b.afterDiff%2 == 1) {
        swap(p.evenNum, p.oddNum);
        swap(p.evenSum, p.oddSum);
      }
  ...
  foreach(i; 0..Q) {
    ...
    l--;
    if (q == 1) {
      seg.update(l, r, R(true, false, 0));
    } else if (q == 2) {
      seg.update(l, r, R(false, false, x));
    } else {
      P p = seg.query(l, r);
      writeln(p.evenSum + p.oddSum);
    }
  }
}
